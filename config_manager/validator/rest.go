...
func validateBlockData(
	...
	for _, endpoint := range *restValidities {
		...
		for height, blockData := range endpoint.blockData {
			if blockData == nil {
				continue
			}
	...
	for url, endpoint := range *restValidities {
		...
		for height, blockData := range endpoint.blockData {
			if blockData == nil {
				...
				log.Printf("Missing block data for %s at height %d, minus %d points",
					endpoint.Endpoint.URL, height, mismatchPenalty)
				continue
			}
			...
			for _, check := range checks {
				if check.expected != check.actual {
					...
					log.Printf("Mismatch found for %s at height %d (%s): expected %v, got %v, minus %d points",
						endpoint.Endpoint.URL, height, check.name, check.expected, check.actual, mismatchPenalty)
				}
			}
		}
		...
		if endpoint.points < 60 {
			endpoint.valid = false
			log.Printf("Endpoint %s marked invalid due to low points: %d", endpoint.Endpoint.URL, endpoint.points)
		}
