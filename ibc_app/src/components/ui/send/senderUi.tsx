...
export default function SendUIOptimized({
  ...
  const getTokensForChain = useCallback((chainId: string): ClientToken[] => {
    const chain = getChainById(chainId);
    if (!chain) return [];
    return [...chain.native_tokens, ...chain.ibc_tokens];
  }, [getChainById]);

  const getConnectedChains = useCallback((chainId: string) => {
    const chain = getChainById(chainId);
    if (!chain) return [];
        ...
        <div className="card bg-primary text-primary-content shadow-xl">
          <div className="card-body">
            ...
            <div className="space-y-3">
              <p><strong>From:</strong> {sendChainData?.name} ({sendToken})</p>
              <p><strong>To:</strong> {receiveChainData?.name} ({receiveToken || "Auto"})</p>
              <p><strong>Amount:</strong> {amount} {sendToken}</p>
              
              <div className="divider"></div>
              
              <div className="bg-primary-content/20 p-4 rounded-lg">
                <h3 className="font-bold mb-2">Route Information</h3>
                <p><strong>Route Type:</strong> {routeInfo.route_type}</p>
                <p><strong>Steps:</strong> {routeInfo.steps.length}</p>
                {routeInfo.supports_pfm && (
                  <p className="text-success">✓ Supports PFM (Single Transaction)</p>
                )}
                {routeInfo.estimated_time_seconds && (
                  <p><strong>Est. Time:</strong> ~{routeInfo.estimated_time_seconds}s</p>
                )}
                {routeInfo.total_fees_estimate && (
                  <p><strong>Est. Fees:</strong> {routeInfo.total_fees_estimate}</p>
                )}
                
                {routeInfo.broker_swap && (
                  <div className="mt-2">
                    <p className="text-warning">⚠ Requires DEX swap on {routeInfo.broker_swap.broker_chain_id}</p>
                    <p className="text-sm">Output: ~{routeInfo.broker_swap.estimated_output}</p>
                  </div>
                )}
              </div>
            </div>
