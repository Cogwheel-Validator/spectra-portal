...
export default function ChainSection(props : ChainSelectionProps) : ReactElement {
    ...
    // Helper functions
    const getChainById = useCallback((id: string): ClientChain | undefined => {
        return config.chains.find((chain) => chain.id === id);
    }, [config]);

    const getTokensForChain = useCallback((id: string): ClientToken[] => {
        const chain = getChainById(id);
        if (!chain) return [];
        return [...chain.native_tokens, ...chain.ibc_tokens];
    }, [getChainById]);

    const getSendableTokens = useCallback((fromId: string, toId: string): string[] => {
    ...
    const allAvailableTokens = useMemo(() => getTokensForChain(chainId), [chainId, getTokensForChain]);
    ...
    const chainSelectPlaceholder: string = isFromChain ? "Select source chain" : "Select destination chain";
    const tokenSelectPlaceholder: string = isFromChain ? "Select token to send" : "Select token to receive (optional)";
    ...
    <div className="card bg-slate-700/50 shadow-xl text-base-content">
      <div className="card-body">
        <div className="flex flex-row justify-between">
            ...
            {chainData ? (
                <Link href={chainData.explorer_url} target="_blank" rel="noopener noreferrer" className="tooltip" data-tip="Chain Explorer">
          ...
          <div className="flex items-center justify-center gap-2">
            <Image
              ...
              className={`rounded-full size-20 bg-slate-900 
                ${isFromChain ? "border-[3px] border-orange-500 shadow-xl shadow-orange-800/70" : 
